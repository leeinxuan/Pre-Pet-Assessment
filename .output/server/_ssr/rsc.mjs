...
function getDraftSecret() {
	return "52b2bb63-ef04-42c9-931b-e88277f61a74";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "60e92e9f-8f43-4fae-a342-2b42b9b44b59";
	}
};
...
}, "ee0430dd8bd8", "ForbiddenBoundary");
...
}, "ee0430dd8bd8", "UnauthorizedBoundary");
/**
...
}, "431ddbe7c781", "LayoutSegmentProvider");
/**
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "60e92e9f-8f43-4fae-a342-2b42b9b44b59") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "60e92e9f-8f43-4fae-a342-2b42b9b44b59",
		rootBoundaryId
	});
}
/**
...
async function saveSharedAssessmentResult(result) {
	const db = await ensureResultsTable();
	const id = crypto.randomUUID().replaceAll("-", "");
	await db.prepare("INSERT INTO shared_assessment_results (id, payload, created_at) VALUES (?, ?, ?)").bind(id, JSON.stringify(result), Date.now()).run();
	return id;
}
async function readSharedAssessmentResult(id) {
	if (!/^[a-f0-9]{32}$/.test(id)) return null;
	const row = await (await ensureResultsTable()).prepare("SELECT payload FROM shared_assessment_results WHERE id = ? LIMIT 1").bind(id).first();
	if (!row) return null;
	try {
...
function isShortText(value, maxLength = 500) {
	return typeof value === "string" && value.length <= maxLength;
}
function isTextList(value, maxItems = 30) {
	return Array.isArray(value) && value.length <= maxItems && value.every((item) => isShortText(item));
}
function isSharedResult(value) {
	if (!value || typeof value !== "object") return false;
	const result = value;
	return result.version === 1 && isShortText(result.createdAt, 40) && !Number.isNaN(Date.parse(result.createdAt)) && isShortText(result.petName, 40) && isShortText(result.breedId, 80) && isShortText(result.breedLabel, 80) && isShortText(result.readinessLevel, 100) && typeof result.preparation?.roomCompletion === "number" && result.preparation.roomCompletion >= 0 && result.preparation.roomCompletion <= 100 && typeof result.preparation.hazardsComplete === "boolean" && typeof result.preparation.transportComplete === "boolean" && typeof result.costs?.simulatedTotal === "number" && typeof result.costs.emergencyReserve === "number" && typeof result.costs.suggestedTotal === "number" && isTextList(result.preparedItems) && isTextList(result.itemsToConfirm) && Array.isArray(result.discussionTopics) && result.discussionTopics.length <= 20 && result.discussionTopics.every((topic) => isShortText(topic?.id, 100) && isShortText(topic?.title, 300) && isShortText(topic?.topic, 150) && isTextList(topic?.knowledgePoints, 10)) && typeof result.committed === "boolean";
}
async function POST(request) {
	try {
		const result = await request.json();
		const serialized = JSON.stringify(result);
		if (!isSharedResult(result) || serialized.length > 5e4) return Response.json({ error: "結果資料格式不正確" }, {
			status: 400,
		...
		return Response.json({ error: "目前無法產生分享連結，請稍後再試。" }, {
			status: 500,
			headers: { "cache-control": "no-store" }
		});
	}
...
async function GET(_request, { params }) {
	try {
		...
		if (!result) return Response.json({ error: "找不到這份結果" }, {
			status: 404,
		...
		return Response.json({ error: "目前無法讀取這份結果" }, {
			status: 500,
			headers: { "cache-control": "private, no-store" }
		});
	}
...
var routes = [
	...
	},
	{
		...
		ids: {
			"route": "route:/api/results",
			"page": null,
			"routeHandler": "route-handler:/api/results",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/results/:id",
			"page": null,
			"routeHandler": "route-handler:/api/results/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/results/:id",
			"page": "page:/results/:id",
			"routeHandler": null,
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
...
var generateStaticParamsMap = { "/results/:id": null };
