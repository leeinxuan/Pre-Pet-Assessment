...
var client_references_default = {
	"257243a48472": async () => {
		const m = await import("./page-Bi47xeGS.mjs");
		return { get "default"() {
			return m["default"];
		} };
	},
	"431ddbe7c781": async () => {
		const m = await import("./layout-segment-context-oEXhtMCv.mjs");
	...
	"6efdf509a785": async () => {
		const m = await import("./page-CghZRaeh.mjs");
		return { get "default"() {
			return m["default"];
		} };
	},
	"ee0430dd8bd8": async () => {
		const m = await import("./error-boundary-Cvh3qkjL.mjs");
		return {
			get "ErrorBoundary"() {
