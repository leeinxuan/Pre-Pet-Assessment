...
| 接回家情境 | 第一日適應：安靜等狗探索 | 第一次來到新家：關好門窗，讓貓自行走出外出籠並探索 | 單選情境＋影片／插圖 |
...
| 品種的考驗 | 各犬種兩題，如換毛、雨天排泄、口腔、訓練 | 橘貓／虎斑貓各三題：互動、垂直空間與飲食管理 | 依目前貓題庫的三題序列＋素材 |
| 忙碌備援 | 請家人／朋友，確認時間、意願、照護知識、緊急聯絡 | 題名「臨時晚歸，誰來接手？」；完全同構，交接內容換成食水、砂盆、環境巡視、陪玩與觀察；不設定固定獨處時數 | 共用忙碌題元件＋文字／素材 |
| 醫療情境 | 健康變化記錄與聯絡獸醫 | 「牠突然變得不太一樣」：食慾、躲藏、尿便等多項改變時，記錄前後變化、準備外出籠與資料並聯繫獸醫；不自行診斷或給藥 | 四選一情境＋文字／素材 |
| 高齡活動 | 防滑、低入口睡墊、碗位、保暖、活動調整 | 11 歲高齡情境：高度適中的砂盆與地墊、階梯式設施、保暖休息處、半年健檢與每週量體重／日常趨勢記錄 | 四選一情境＋素材 |
...
| `cat-first-safe-room` | `/assets/cat/life/first-safe-room.mp4` | 影片／圖片 | 第一次來到新家題 | `［待補素材：貓咪來到新家後自主探索］` |
...
| 新家安靜空間背景 | `cat-safe-room`、`cat-safe-room-mobile`，橫式與直式背景 | 安靜小房間：關閉且有防護的窗、外出籠、躲藏／休息空間、食水區、與食水分開的砂盆區；畫面中央留出可放置用品的空位 | **空房版**與**完成布置版**各一張；物件最好另切透明圖，不要全部畫死在背景 | 房間布置、到家、新家題 |
...
|---|---|---|---|
| 接回家過場 | `cat-arrival-transition`，無聲 MP4，約 6–8 秒，可循環 | 車子停下、外出籠被平穩帶入家門、鏡頭切到新家中安靜空間的門口；不拍貓在籠內驚慌 | 影片字幕：`「到家後，先帶牠到準備好的安靜空間。」`；結尾停在可進入題目的門口畫面 |
| 第一次來到新家 | `cat-first-safe-room`，無聲 MP4 或 3 張連續插圖 | 外出籠放穩、門打開後人退後等待、貓自行走出並選擇躲藏／探索；不抱出、不拉扯 | 約 8–10 秒；選項用同一畫面分出「讓牠自行離籠」與錯誤的「伸手拖出」圖示；字幕說明低干擾等待 |
...
|---|---|---|---|
| 晚上還很有精神 | 客廳夜景，貓注意安全實體玩具；畫面保留四個選項圖示位置 | 逗貓棒／玩偶／球與安靜休息處 | 雷射光點以簡明禁用圖示呈現，不呈現傷害 |
| 抓沙發 | 沙發側邊與抓板並列，讓玩家看見替代選擇 | 在合適位置的抓板、跳台或抓柱 | 去爪、磨牙、剃鬍鬚以三個清楚禁用圖示呈現；不呈現侵入性操作 |
...
| 健康變化 | `cat-health-change`，橫式插圖 | 貓安靜躲在休息處，照顧者記下食慾、飲水、尿便、活動與躲藏等變化；旁邊有外出籠、時間紀錄與資料夾圖示 | 唯一正確處置為「完整紀錄＋準備外出籠／資料＋聯繫獸醫」；不得出現疾病名稱、診斷結果或藥物 |
| 高齡空間 | `cat-senior-room`，橫式插圖 | 11 歲貓自然步行；高度適中的額外砂盆與地墊、階梯式跳台、被褥／保暖休息處；角落為半年健檢與每週量體重紀錄卡 | 唯一正確處置整合環境與追蹤；錯誤選項對應「只買保健品／只放軟墊／維持高跳台」三張簡明圖示 |
